...
class UserController extends AppController
{
    ...
    public function actionCreate()
    {
        ...
        if (!$info) {
            Yii::$app->session->setFlash('error', 'Foydalanuvchi rolini saqlash paytida xatolik yuz berdi.');
        }

        $user->images = UploadedFile::getInstance($user, 'images');
        if( $user->images ){
            $user->upload();
        }
    ...
    public function actionUpdate($id)
    {
        ...
        if (!$info) {
            Yii::$app->session->setFlash('error', 'There was some error while saving user role.');
        }
    ...
    public function actionDelete($id)
    {
        ...
        $model->removeImage($filename);
        FileHelper::removeDirectory('upload/store/'.$filename[modelName].'s/'.$filename[modelName].$filename[itemId]);

        // delete user or throw exception if could not
        if (!$this->findModel($id)->delete()) {
            throw new ServerErrorHttpException('Ushbu foydalanuvchini o\'chirib bo\'lmadi.');
        }
        ...
        if (!$info) {
            Yii::$app->session->setFlash('error', 'Foydalanuvchi rolini o\'chirishda xatolik yuz berdi.');
            return $this->redirect(['index']);
        }

        Yii::$app->session->setFlash('success', 'Siz foydalanuvchini va uning rolini muvaffaqiyatli o\'chirib tashladingiz.');
    ...
    protected function findModel($id)
    {
        ...
        if (is_null($model)) {
            throw new NotFoundHttpException('So\'ralgan sahifa mavjud emas.');
        }
