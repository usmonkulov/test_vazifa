...
class AccountActivation extends Model
{
    ...
    public function __construct($token, $config = [])
    {
        if (empty($token) || !is_string($token)) {
            throw new InvalidParamException(Yii::t('yii', 'Hisobni faollashtirish belgisi bo\'sh bo\'lishi mumkin emas.'));
        }

        $this->_user = User::findByAccountActivationToken($token);

        if (!$this->_user) {
            throw new InvalidParamException(Yii::t('yii', 'Hisobni faollashtirish belgisi noto\'g\'ri.'));
        }
