...
class ResetPasswordForm extends Model
{
    ...
    public function __construct($token, $config = [])
    {
        if (empty($token) || !is_string($token)) {
            throw new InvalidParamException(Yii::t('yii', 'Parolni tiklash belgisi bo\'sh bo\'lishi mumkin emas.'));
        }

        $this->_user = User::findByPasswordResetToken($token);

        if (!$this->_user) {
            throw new InvalidParamException(Yii::t('yii', 'Parolni tiklash belgisi noto\'g\'ri.'));
        }
